...
//    private final String secret = "8c23c4aa-c228-448b-a86d-c04077c39603";
    ...
    private String sign(String path, String body, String timestamp) throws Exception {
    ...
    private HttpHeaders headers(String path, String body) throws Exception {
        String ts = now();
        HttpHeaders h = new HttpHeaders();
        h.set("X-Api-Key", apiKey);
        h.set("X-Timestamp", ts);
        h.set("X-Signature", sign(path, body, ts));
        h.setContentType(MediaType.APPLICATION_JSON);
        return h;
    }

    public BalanceResponse getBalance(String cashpointId) throws Exception {
        ...
        HttpEntity<?> req = new HttpEntity<>(headers(path, ""));
    ...
    public TransactionResponse deposit(String cashpointId, int brandId, String playerId, double amount, String currency) throws Exception {
        // FIX: Construct the full path for the signature.
        String path = API_PATH_PREFIX + "/" + cashpointId + "/player/deposit";
        DepositRequest body = new DepositRequest(brandId, playerId, amount, currency);
        ...
        HttpHeaders h = headers(path, json);
    ...
    public TransactionResponse confirmCashout(String cashpointId, String code, long transactionId) throws Exception {
        // FIX: Construct the full path for the signature.
    ...
    public TransactionListResponse getTransactions(String cashpointId, String dateStart, String dateEnd,
                                                   Integer brandId, String playerId, Long transactionId) throws Exception {
        StringBuilder qb = new StringBuilder("dateStart=").append(dateStart).append("&dateEnd=").append(dateEnd);
        if (brandId != null) qb.append("&brandId=").append(brandId);
        if (playerId != null) qb.append("&playerId=").append(playerId);
        if (transactionId != null) qb.append("&transactionId=").append(transactionId);
        String qs = qb.toString();
        // FIX: Construct the full path for the signature, including the query string.
        String path = API_PATH_PREFIX + "/" + cashpointId + "/transactions/list/all?" + qs;
        HttpEntity<?> req = new HttpEntity<>(headers(path, ""));
        String url = FULL_BASE_URL + "/" + cashpointId + "/transactions/list/all?" + qs;
        ResponseEntity<TransactionListResponse> resp = restTemplate.exchange(url, HttpMethod.GET, req, TransactionListResponse.class);
        return resp.getBody();
    }
