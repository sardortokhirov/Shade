...
public class TopUpSimulationRunner implements CommandLineRunner {
    ...
    private void displayInitialState(String phoneNumber) {
        ...
        BigDecimal permanentLimitIncreaseBD = userLimitIncreaseService.getPermanentLimitIncrease(TARGET_CHAT_ID);
        ...
        logger.info("Permanent Limit Increase: {} UZS (precise: {})", permanentLimitIncrease, permanentLimitIncreaseBD);
    ...
    private void processTopUp() {
        ...
        // Calculate expected values (precise decimal)
        BigDecimal topUpPercentage = configurationService.getTopUpDailyLimitIncreasePercentage();
        BigDecimal expectedLimitIncreaseBD = BigDecimal.valueOf(TOP_UP_AMOUNT)
                .multiply(topUpPercentage)
                .divide(BigDecimal.valueOf(100), 8, java.math.RoundingMode.HALF_UP);
        long expectedLimitIncrease = expectedLimitIncreaseBD.setScale(0, java.math.RoundingMode.HALF_UP).longValue();
        
        Long ticketCalculationAmount = configurationService.getTicketCalculationAmount();
        long expectedTickets = TOP_UP_AMOUNT / ticketCalculationAmount;
        
        logger.info("Expected Limit Increase: {} UZS (precise: {}) ({}% of {})", 
                expectedLimitIncrease, expectedLimitIncreaseBD, topUpPercentage, TOP_UP_AMOUNT);
        ...
        logger.info("✓ Permanent limit increased by {} UZS (precise: {})", expectedLimitIncrease, expectedLimitIncreaseBD);
    ...
    private void displayFinalState(String phoneNumber) {
        ...
        BigDecimal topUpPercentage = configurationService.getTopUpDailyLimitIncreasePercentage();
        BigDecimal limitIncreaseBD = BigDecimal.valueOf(TOP_UP_AMOUNT)
                .multiply(topUpPercentage)
                .divide(BigDecimal.valueOf(100), 8, java.math.RoundingMode.HALF_UP);
        long limitIncrease = limitIncreaseBD.setScale(0, java.math.RoundingMode.HALF_UP).longValue();
        ...
        logger.info("Permanent Limit Increased: {} UZS (precise: {})", limitIncrease, limitIncreaseBD);
